...
/*print a grapic bar in direaction and amout, amount is in scale of 20 to 1 in the scale*/
// so [     |o   ]  = 20  so [     |oo   ]  = 40 o == pixcel
void oledWriteGrapicBar(int derection,int amount,int row,int s)
{
	BYTE graphic_byte = 0x18;
	BYTE empte_byte = 0x00;					
	int start = s,i;
	...
	if(s == 48){
		derection = 1;
		for(i = 0 ; i < 53 ; i++){
			oledWriteBYTE1x(empte_byte, row, start);
			start = start + derection;
		}
	...
	while(amount != 0){
		start = start + derection;
		oledWriteBYTE1x(graphic_byte, row, start); 
	amount--;	
	}
	return;
}

void oledWriteCharInPlace(char letter, unsigned char page, unsigned char column){
...
/*put 1 Byte at specific place*/
void oledWriteBYTE1x(BYTE letter, unsigned char page, unsigned char column)
{
	BYTE i;
	page = page + 0xB0;
	WriteCommand(page);
	...
	oledWriteBYTERaw( letter ) ;
	return;
}

/*puting raw BYTE*/
void oledWriteBYTERaw( BYTE byte )
{	
	WriteData(byte);
	return;
}
...
ROM BYTE g_pucFont[95][5] =
{
    ...
    { 0x3c, 0x7e, 0x7e, 0x7e, 0x3c }, // !   0x21
    { 0x00, 0x07, 0x00, 0x07, 0x00 }, // "   0x22
    { 0xff, 0x81, 0xbd, 0x81, 0xff }, // #   0x23
    { 0x7e, 0x7f, 0x7f, 0x7f, 0x7e }, // $   0x24
    ...
    { 0x00, 0x00, 0xff, 0x00, 0x00 }, // ?   0x3F
    { 0x0e, 0x11, 0x11, 0x11, 0x0e }, // @   0x40
    { 0x7e, 0x11, 0x11, 0x11, 0x7e }, // A   0x41
    { 0x7f, 0x49, 0x49, 0x49, 0x36 }, // B   0x42
    { 0x3e, 0x41, 0x41, 0x41, 0x3e }, // C   0x43
    ...
    { 0x08, 0x08, 0x08, 0x08, 0x08 }, // J   0x4A
    ...
    { 0xf8, 0x04, 0x02, 0x04, 0xf8 }, // a   0X61
    ...
    { 0x2a, 0x55, 0xaa, 0x55, 0x2a }, // v   0X76
